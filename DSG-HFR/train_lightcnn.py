...
parser.add_argument('--vis_root', type=str, default="./tufts_dataset/vis/data/")
parser.add_argument('--inf_root', type=str, default="./tufts_dataset/inf/data/")
...
parser.add_argument('--model_saving_path', type=str, default="./trained_model/lightcnn/")
...
def pre_train(train_loader, model, optimizer, epoch):
    ...
    for vis, inf, l in tqdm(train_loader):
        ...
        logits_vis, _ = model(vis, l, 0)
        logits_inf, _ = model(inf, l, 0)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for vis, inf, l in tqdm(train_loader):
        ...
        logits_vis, fc_vis = model(vis, l, 0)
        logits_inf, fc_inf = model(inf, l, 0)
...
for _ in range(1):
    ...
    model = LightCNN_29v2_cosface(args.pretrained_lightcnn_path, device, num_classes + 1).to(device)
    ...
    for epoch in range(args.pretrain_epoch):
        ...
        torch.save(model.state_dict(), args.model_saving_path + "Lightcnn_pretrain.pt")
    ...
    for epoch in range(args.train_epoch):
        ...
        torch.save(model.state_dict(), args.model_saving_path + "Lightcnn_train_" + str(epoch) + ".pt")
